...
begin
  ...
  v_local_now := timezone('Asia/Ho_Chi_Minh', now());
  v_start_local := date_trunc(p_period, v_local_now);
...
revoke execute on function public.admin_get_dashboard_stats(text)
from public, anon;

grant execute on function public.admin_get_dashboard_stats(text)
to authenticated;
